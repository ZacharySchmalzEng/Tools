...
if ($Help -or -not $RunAny) {
    ...
    Write-Host " Windows 11 Pro Environment Setup Script v5.6" -ForegroundColor White
    Write-Host "===============================================================" -ForegroundColor Cyan
    Write-Host "Usage: .\InstallScript_v5.6.ps1 [Options]"
    Write-Host ""
    ...
    Write-Host "  -Dev       Enables OS developer features (WSL, Windows Sandbox, OpenSSH Client)"
...
if ($DualBoot) {
    ...
    Write-Host "--- Configuring Scheduled Task for Fedora WSL Mount... ---" -ForegroundColor Yellow
    try {
        ...
        $MountScriptPath = Join-Path -Path $CustomScriptsDir -ChildPath "Mount-Fedora.ps1"
...
if ($RunSoftware) {
    ...
    if ($Maker) {
        $packages += "PrusaResearch.PrusaSlicer", "SoftFever.OrcaSlicer", "Bambulab.Bambustudio", "Autodesk.Fusion360"
    }
