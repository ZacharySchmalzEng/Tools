...
.DESCRIPTION
    ...
    - Installation of the NVIDIA App (preserving existing configurations).
    - Special handling for Blizzard Battle.net silent installation.
    - Enables WSL, Windows Sandbox, and OpenSSH (Client & Server).
...
# 11. --- Install NVIDIA App (Check & Download only if missing) ---
...
try {
    ...
    if (Test-Path $NvAppExe) {
        ...
        if (Test-Path $InstallerPath) {
            ...
            Write-Host "Installer not found locally. Using URL: $url"
...
Write-Host "NVIDIA App version 11.0.5.266 processed."
